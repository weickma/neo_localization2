  ...
  void smooth_33_1()
  {
    ...
    for(int y = 0; y < m_size_y; ++y) {
      for(int x = 0; x < m_size_x; ++x) {
        float sum = 0;
        for(int j = -1; j <= 1; ++j) {
          const int y_ = ((y + j > 0 ? y + j:0) < m_size_y - 1) ? (y + j > 0 ? y + j:0) : m_size_y - 1;
          for(int i = -1; i <= 1; ++i) {
            const int x_ = ((x + i > 0 ? x + i:0) < m_size_x - 1) ? (x + i > 0 ? x + i:0) : m_size_x - 1;
  ...
  std::shared_ptr<GridMap<T>> downscale()
  {
    ...
    for(int y = 0; y < m_size_y / 2; ++y) {
      for(int x = 0; x < m_size_x / 2; ++x) {
        float sum = 0;
        for(int j = -1; j <= 2; ++j) {
          const int y_ = ((y * 2 + j > 0 ? y * 2 + j:0) < m_size_y - 1) ? (y * 2 + j > 0 ? y * 2 + j:0) : m_size_y - 1;
          for(int i = -1; i <= 2; ++i) {
            const int x_ = ((x * 2 + i > 0 ? x * 2 + i:0) < m_size_x - 1) ? (x * 2 + i > 0 ? x * 2 + i:0) : m_size_x - 1;
