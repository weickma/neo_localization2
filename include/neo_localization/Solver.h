...
public:
        ...
        const std::vector<scan_point_t>& points)
  {
    ...
    for(const auto& point : points)
    {
      ...
      double q0 = P(0,0) * point.x  + P(0,1) * point.y + P(0,2) * 1;
      double q1 = P(1,0) * point.x  + P(1,1) * point.y + P(1,2) * 1;

      const float grid_x = grid.world_to_grid(q0);
      const float grid_y = grid.world_to_grid(q1);
...
protected:
  ...
  void integrate(const float p_x, const float p_y, const float r_i, const float dx, const float dy)
  {
    ...
    const float J_yaw =   dx * (-sin(pose_yaw) * p_x - cos(pose_yaw) * p_y)
              + dy * ( cos(pose_yaw) * p_x - sin(pose_yaw) * p_y);
                              ...
                              const Matrix<double, 3, 1>& pose)
{
  ...
  for(const auto& point : points)
  {
    ...
    const float ddyaw = (sin(dir) * ddx + cos(dir) * ddy) * length;
